...
import { localIdeas } from '../generator.js';
import { aiEnabled, aiIdeas } from '../ai.js';
import { esc, typeLabel, typeIcon, busy, toast } from '../ui.js';
...
export function render(el, { navigate }) {
  ...
  el.innerHTML = `
      ...
      <div class="row">
        ...
        <label class="field" style="min-width:220px">Post-Typ
          <select id="f-type"><option value="">Alle meine Typen</option>
            ${POST_TYPES.map((t) => `<option value="${t.id}" ${filter.type === t.id ? 'selected' : ''}>${t.icon} ${t.label}</option>`).join('')}
          </select></label>
    ...
    ${list.length ? `<div class="grid g3">${list.map((i) => card(i, s)).join('')}</div>`
      : `<div class="empty"><div class="big">💡</div><p>Noch keine gespeicherten Ideen. Speichere Vorschläge mit ☆.</p></div>`}`;
...
function card(i, s) {
  ...
  return `<div class="idea-card">
    <div class="meta">${pillar ? `<span class="tag blue">${esc(pillar.name)}</span>` : ''}<span class="tag">${typeIcon(i.type)} ${esc(typeLabel(i.type))}</span>${i.source === 'ki' ? '<span class="tag green">KI</span>' : ''}</div>
