...
import { esc, typeIcon, typeLabel, startOfWeek, addDays, sameDay, DAY_NAMES } from '../ui.js';
...
export function render(el, { navigate }) {
  ...
  el.innerHTML = `
    <div class="card" style="margin-bottom:20px">
      ...
      <div class="cal">
        ${days.map((d) => {
          ...
          return `<div class="cal-day ${sameDay(d, today) ? 'today' : ''}">
            <div class="dh"><span>${DAY_NAMES[(d.getDay() + 6) % 7]} ${d.getDate()}.</span>${slot && !posts.length ? '<span title="Laut Wochenplan">📌</span>' : ''}</div>
            ${posts.map((p) => `<div class="cal-post ${p.status === 'veröffentlicht' ? 'published' : ''}" data-open="${p.id}">
              <div class="t">${new Date(p.scheduledAt).toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit' })} · ${typeIcon(p.type)}</div>
      ...
      <p class="tiny" style="margin:12px 0 0">📌 = Vorschlag aus deinem Wochenplan (${s.voice.perWeek} Posts/Woche). LinkedIn-Posting erfolgt manuell: Text kopieren und als veröffentlicht markieren.</p>
    </div>
    ...
    <div class="card">
      ...
      ${drafts.length ? drafts.map((d) => {
        ...
        return `<div class="draft-row">
          <span style="font-size:22px">${typeIcon(d.type)}</span>
      ...
      }).join('') : '<div class="empty"><div class="big">📝</div><p>Noch keine Entwürfe.</p><a class="btn btn-primary" href="#/write?new=1">Ersten Post schreiben</a></div>'}
    </div>`;
